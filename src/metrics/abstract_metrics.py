...
def safe_mse_loss(preds: torch.Tensor, target: torch.Tensor):
    """
    Berechnet MSE zwischen preds und target.
    Wenn target leer ist, wird -1 zurückgegeben.
    """
    if target.numel() == 0:
        return torch.tensor(-1.0, device=preds.device)
    else:
        return F.mse_loss(preds, target, reduction="sum")
import torch
...
class MeanSquaredErrorMetric(Metric):
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor, weight: torch.Tensor = None):
        if weight is not None:
            # Weighted MSE
            output = safe_mse_loss(preds, target)
            if output.dim() > 1:
