...
class SynthDataset(InMemoryDataset):
    """
    Dataset for synthetic molecules from synth2.csv
    """
    atom_encoder = None
    atom_decoder = None
    atom_types = None
    ...
    def processed_dir(self):
        return osp.join(self.root, 'synth2', 'processed')
    ...
    def download(self):
        ...
        target_path = osp.join(self.raw_dir, 'synth2.csv')
    ...
    def process(self):
        ...
        #types_list = [
        #    'Al', 'As', 'B', 'Br', 'C', 'Cl', 'Co', 'Cr', 'F', 'Ga', 'Ge',
        #   'H', 'Hg', 'I', 'Mn', 'Mo', 'N', 'O', 'P', 'Pb', 'Re', 'S', 'Sb',
        #    'Se', 'Si', 'Sn', 'Te', 'U', 'W'
        #]

        #types = {atom: i for i, atom in enumerate(types_list)} 
        types = {
            'C': 0, 'N': 1, 'O': 2, 'F': 3, 'P': 4,
            'S': 5, 'Cl': 6, 'Br': 7, 'I': 8,  
        }
        
        print("Detected atom types:", types)
        ...
        for split in ['train', 'val', 'test']:
            ...
            for idx in indices:
                ...
                dist_columns = ['dist_ch0_cl0', 'dist_ch0_cl1', 'dist_ch1_cl2', 'dist_ch1_cl3']
...
class SynthDataModule(MolecularDataModule):
    def __init__(self, cfg):
        ...
        self.csv_path = "/hkfs/work/workspace_haic/scratch/rx3495-workspace_C/DeFoG/data/synth2/raw/synth2.csv"
        target = getattr(cfg.general, "target", None)
        regressor = getattr(cfg.general, "conditional", None)
        dynamic = getattr(cfg.general, "dynamic", True)
...
class SynthInfos(AbstractDatasetInfos):
    def __init__(self, datamodule, cfg, recompute_statistics=False):
        ...
        if self.remove_h:
            ...
            # Use datamodule stats if recompute_statistics=True
            if recompute_statistics:
                ...
                '''
                synth data2
                self.n_nodes = torch.tensor([0.0000, 0.0000, 0.0002, 0.0003, 0.0004, 0.0005, 0.0008, 0.0025, 0.0136,
                    0.0161, 0.0191, 0.0223, 0.0234, 0.0254, 0.0478, 0.0541, 0.0576, 0.0522,
                    0.0584, 0.0654, 0.0730, 0.0732, 0.0699, 0.0598, 0.0665, 0.0475, 0.0499,
                    0.0267, 0.0262, 0.0162, 0.0143, 0.0098, 0.0072])
                
                
                self.node_types = torch.tensor([0.7749, 0.1133, 0.0623, 0.0085, 0.0010, 0.0210, 0.0140, 0.0042, 0.0008])
                            
                self.edge_types = torch.tensor([8.9411e-01, 7.3804e-02, 3.1231e-02, 8.5294e-04]) 
                
              
                valency_values  = torch.tensor([
                    0.0000, 0.1230, 0.1999, 0.4218, 0.2539, 0.0010, 0.0004, 0.0000, 0.0000,
                ])  
                '''
                #real data
                self.n_nodes = torch.tensor([0.0000e+00, 0.0000e+00, 0.0000e+00, 0.0000e+00, 0.0000e+00, 0.0000e+00,
                    1.6956e-05, 0.0000e+00, 1.1869e-04, 3.5607e-04, 7.1214e-04, 3.1538e-03,
                    4.7307e-03, 6.8671e-03, 1.0614e-02, 1.7498e-02, 2.6332e-02, 3.5048e-02,
                    4.5560e-02, 5.7717e-02, 7.1112e-02, 8.2422e-02, 7.4470e-02, 8.3999e-02,
                    9.3443e-02, 8.8950e-02, 7.4114e-02, 6.5297e-02, 4.0558e-02, 3.1538e-02,
                    2.5264e-02, 2.0449e-02, 1.5684e-02, 1.0546e-02, 7.0875e-03, 4.0524e-03,
                    1.7125e-03, 5.7650e-04])
                
                
                self.node_types = torch.tensor([7.3647e-01, 1.2185e-01, 9.9985e-02, 1.3802e-02, 2.1397e-05, 1.8061e-02,
                    7.5631e-03, 2.1134e-03, 1.4073e-04])
                            
                self.edge_types = torch.tensor([9.0672e-01, 6.9301e-02, 2.3739e-02, 2.3750e-04]) 
                
              
                valency_values  = torch.tensor([0.0000e+00, 1.1374e-01, 3.0429e-01, 3.5014e-01, 2.2696e-01, 2.0574e-05,
                        4.8555e-03])  
                
                self.valency_distribution = torch.zeros(109)
                for i, val in enumerate(valency_values):
                    if i < len(self.valency_distribution):
                        self.valency_distribution[i] = val
                print(f"Valency distribution length: {len(self.valency_distribution)}")  
                print(f"Valency distribution sum: {self.valency_distribution.sum()}")  
