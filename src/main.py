...
# put this at the very top of your entrypoint (before `import torch`)
...
from typing import _SpecialForm, _GenericAlias, _UnionGenericAlias, _SpecialGenericAlias
import typing

# which can raise AttributeError during torch.load. Add these private typing
# classes to safe globals when available so unpickling works for older/newer
# checkpoints.
# safe_typing vorbereiten
# Nur ausführen, wenn add_safe_globals existiert (PyTorch ≥ 2.2)
if hasattr(torch.serialization, "add_safe_globals"):

    # OmegaConf, NumPy, builtins, PyTorch Klassen
    ...
    # Interne Typen für alte Pickles
    safe_typing = []
    for _name in ("_SpecialForm", "_GenericAlias", "_UnionGenericAlias", "_SpecialGenericAlias"):
        if hasattr(typing, _name):
            safe_typing.append(getattr(typing, _name))

    if safe_typing:
        torch.serialization.add_safe_globals(safe_typing)


# PyTorch < 2.2: alles übersprungen, kein Fehler

def add_project_safe_globals():
    """
    Dynamically add all project classes (models.*, datasets.*, metrics.*, analysis.*)
    so checkpoints can unpickle them.
    """
    import sys
    safe_classes = []
    ...
    for module_name, module in sys.modules.items():
        if any(module_name.startswith(prefix) for prefix in
               ("models.", "datasets.", "metrics.", "analysis.")):
            try:
...
def main(cfg: DictConfig):
    ...
    if dataset_config["name"] in [
    ...
    elif dataset_config["name"] in ["qm9", "guacamol", "moses", "zinc","zinc_det", "aqsoldb", "synth", "synth2", "synth3"]:
        ...
        elif "zinc_det" in dataset_config["name"]:
        ...
        elif "synth3" in dataset_config["name"]:
            from datasets import synth3_dataset

            datamodule = synth3_dataset.SynthDataModule(cfg)
            dataset_infos = synth3_dataset.SynthInfos(datamodule=datamodule, cfg=cfg)
            dataset_smiles = synth3_dataset.get_smiles(
                cfg=cfg,
                datamodule=datamodule,
                dataset_infos=dataset_infos,
                evaluate_datasets=False,
            )    
        elif "synth2" in dataset_config["name"]:
            from datasets import synth2_dataset

            datamodule = synth2_dataset.SynthDataModule(cfg)
            dataset_infos = synth2_dataset.SynthInfos(datamodule=datamodule, cfg=cfg)
            dataset_smiles = synth2_dataset.get_smiles(
                cfg=cfg,
                datamodule=datamodule,
                dataset_infos=dataset_infos,
                evaluate_datasets=False,
    ...
    # Optional: force specific conditioning values via config/CLI
    # Usage (CLI): +general.condition_values="-0.5,0.12"  (order must match cfg.general.target)
    from omegaconf import ListConfig

    force_cond = getattr(cfg.general, "condition_values", None)
    if force_cond is not None:
        try:
            if isinstance(force_cond, (list, tuple, ListConfig)):
                force_cond_list = [float(x) for x in force_cond]

            elif isinstance(force_cond, str):
                nums = re.findall(r"-?\d+(?:\.\d+)?", force_cond)
                if not nums:
                    raise ValueError("No numeric values found")
                force_cond_list = [float(n) for n in nums]

            else:
                force_cond_list = [float(force_cond)]

            model_kwargs["forced_conditions"] = torch.tensor(
                force_cond_list, dtype=torch.float
            )

            print(f"Using forced condition values: {force_cond_list}")

        except Exception as e:
            print(f"Could not parse cfg.general.condition_values: {e}")
