...
def compute_logp(mol):
    """
    Berechnet RDKit cLogP (Crippen).
    Erwartet ein sanitisiertes RDKit-Mol.
    """
    try:
...
class BasicMolecularMetrics(object):
    ...
    def detect_p(self,mol: Chem.Mol) -> list[tuple[int, ...]]:
        """Return all C#N-group matches in *mol* using a generic SMARTS pattern."""

        pattern = Chem.MolFromSmarts("[nX2]")
        return list(mol.GetSubstructMatches(pattern))
    
    def detect_O(self,mol: Chem.Mol) -> list[tuple[int, ...]]:
        """Return all =O-group matches in *mol* using a generic SMARTS pattern."""
    ...
    def cond_sample_metric(self, samples, input_properties):
        ...
        if self.args.dataset.name == "zinc_det" or self.args.dataset.name == "synth" or self.args.dataset.name == "synth2" or self.args.dataset.name == "synth3":
            mols_logp = []
            mols_qed = []
            mols_num_atoms = []
            mols_nh2 = []
            ...
            for i, sample in enumerate(samples):
                ...
                matches = self.detect_p(mol)
                mols_p.append(len(matches))
                matches = self.detect_O(mol)
                mols_O.append(len(matches))
            ...
            if self.args.general.dynamic:
                separators = [' ', ',', ';', ':', '|']
                separator = ' '
                for sep in separators:
                    if sep in self.args.general.target:
                        separator = sep
                        break
                ...
                for idx, key in enumerate(keys):
                    ...
                    """    
                    elif key == "label" and (self.args.dataset.name == "synth2" or self.args.dataset.name == "synth3"):
                        ...
                        o_percentage = (torch.FloatTensor(mols_O) > 0).float().mean().item() * 100
                        p_percentage = (torch.FloatTensor(mols_p) > 0).float().mean().item() * 100
                        cond_results["oh_percentage"] = oh_percentage
                        cond_results["nh2_percentage"] = nh2_percentage
                        cond_results["=O_percentage"] = o_percentage
                        cond_results["pyr_percentage"] = p_percentage
                    elif key == "label":
                        oh_percentage = (torch.FloatTensor(mols_oh) > 0).float().mean().item() * 100
                        nh2_percentage = (torch.FloatTensor(mols_nh2) > 0).float().mean().item() * 100
                        cond_results["oh_percentage"] = oh_percentage
            ...
            print("LOGP and num_atoms conditional generation Zinc")
            return cond_results.get("logp_mae", -1), validity_rate, cond_results
        print("conditional generation QM9 with PSI4")
        ...
        if self.args.general.dynamic and (self.args.dataset == "zinc_det" or self.args.dataset == "synth" or self.args.dataset == "synth2" or self.args.dataset == "synth3"):
            ...
            if "logp" in keys:
                logp = [compute_logp(build_molecule(atom_types, edge_types, self.dataset_info.atom_decoder)) for atom_types, edge_types in generated]
            ...
            if "num_atoms" in keys:
                num_atoms = [len(atom_types) for atom_types, edge_types in generated]
                num_atoms = torch.FloatTensor(num_atoms)
                mae = self.cond_val(
        ...
        return mae, self.num_valid_molecules / self.num_total , cond_results
    ...
    def evaluate(self, generated, input_properties=None, test=False):
        ...
        if input_properties is not None:
            cond_mae, cond_val, cond_results = self.cond_sample_metric(generated, input_properties)
        ...
        return (
            ...
            all_smiles,
            [cond_mae, cond_val, cond_results],
        )
...
def compute_molecular_metrics(
    ...
    print("RDKit new conds:", rdkit_metrics[-1][2])
    ...
    dic = {
        ...
        "cond_results": rdkit_metrics[-1][2]
    }
    if wandb.run:
        dic = dic
        wandb.log(dic)
