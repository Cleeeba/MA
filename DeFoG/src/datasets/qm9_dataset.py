...
class SelectBothTransform:
    def __call__(self, data, return_y=False):
        # Stack the two target columns as separate feature columns, preserving
        # a shape of (N, 2) for batched label tensors. Using `hstack` on 1-D
        # tensors would concatenate them end-to-end producing wrong shapes.
        if return_y:
            return torch.stack((data.y[..., 3], data.y[..., 5]), dim=-1)
        data.y = torch.stack((data.y[..., 3], data.y[..., 5]), dim=-1)
        return data
    
class SelectDynamicTransform:
    """
    Dynamically select QM9 target properties from a string argument.

    Example usage with new boolean dynamic flag:
        cfg.general.dynamic = true
        cfg.general.target = "mu homo"  # or "mu,homo"
    """

    # QM9 target index mapping (based on standard QM9 ordering)
    TARGET_MAP = {
        "a": 0,
        "b": 1,
        "c": 2,
    ...
    def __init__(self, dynamic: str):
        if not isinstance(dynamic, str):
            raise TypeError("dynamic must be a string, e.g. 'mu homo'")
        ...
        if unknown:
            raise ValueError(f"Unknown QM9 targets: {unknown}")
...
class QM9DataModule(MolecularDataModule):
    def __init__(self, cfg):
        ...
        # Wenn dynamic=True, verwende SelectDynamicTransform mit target string
...
class QM9infos(AbstractDatasetInfos):
    def __init__(self, datamodule, cfg, recompute_statistics=False):
        ...
        if cfg.general.conditional:
            #self.test_labels = cfg.dataset["test"].data.y
            
            
            # QM9DataModule sets `self.test_labels` during initialization
            # (see `QM9DataModule.__init__`). Read the labels from the
            # provided datamodule instead of from the config object.
