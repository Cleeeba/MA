...
class GraphDiscreteFlowModel(pl.LightningModule):
    def __init__(
        ...
        # Optional ACT (ml-act) guidance integration. If configured, attempt to load
        # ml-act ModelWithHooks and prepare it so we can temporarily register hooks
        # during sampling to compute intervention-adjusted predictions.
        self.act_model_hooks = None
        try:
            act_cfg = None
            if hasattr(self.cfg, "act") and isinstance(self.cfg.act, dict) or hasattr(self.cfg, "act"):
                act_cfg = self.cfg.act
            if act_cfg is not None and getattr(act_cfg, "enabled", False):
                # Add ml-act package root (assumed to be a sibling folder named 'ml-act')
                ml_act_root = Path(__file__).resolve().parents[2] / "ml-act"
                if str(ml_act_root) not in sys.path:
                    sys.path.insert(0, str(ml_act_root))
                from act.models.model_with_hooks import ModelWithHooks

                device_str = "cuda" if torch.cuda.is_available() else "cpu"
                self.act_model_hooks = ModelWithHooks(module=self.model, device=device_str)
                # Load hooks if folder provided
                try:
                    folder = Path(getattr(act_cfg, "hooks_folder", ml_act_root / "hooks"))
                    module_names = getattr(act_cfg, "module_names", [".*"])
                    hook_type = getattr(act_cfg, "hook_type", "aura")
                    hook_params = getattr(act_cfg, "hook_params", {}) or {}
                    # This will populate self.act_model_hooks.hooks and return hook objects
                    self.act_model_hooks.load_hooks_from_folder(
                        folder=folder, module_names=module_names, hook_type=hook_type, **hook_params
                    )
                except Exception:
                    # If hook loading fails, keep act_model_hooks but without hooks
                    pass
        except Exception:
            # Any issues importing ml-act should not break DeFoG
            self.act_model_hooks = None
    ...
    def sample_p_zs_given_zt(
        ...
        if self.conditional:
            # Compute unconditional prediction (set y to sentinel -1)
            uncond_y = torch.ones_like(y_t, device=self.device) * -1
            noisy_uncond = noisy_data.copy()
            noisy_uncond["y_t"] = uncond_y

            extra_uncond = self.compute_extra_data(noisy_uncond)
            pred_uncond = self.forward(noisy_uncond, extra_uncond, node_mask)

            pred_X_uncond = F.softmax(pred_uncond.X, dim=-1)
            pred_E_uncond = F.softmax(pred_uncond.E, dim=-1)

            G_1_pred_uncond = (pred_X_uncond, pred_E_uncond)

            R_t_X_uncond, R_t_E_uncond = self.rate_matrix_designer.compute_graph_rate_matrix(
                t, node_mask, G_t, G_1_pred_uncond
            )

            # If ACT hooks are configured and loaded, use them to compute an
            # intervention-adjusted prediction, otherwise use the original pred.
            guidance_weight = self.cfg.general.guidance_weight
            if self.act_model_hooks is not None and len(self.act_model_hooks.get_hooks()) > 0:
                try:
                    # Register hooks so they fire during the forward call
                    self.act_model_hooks.register_hooks()
                    extra_act = self.compute_extra_data(noisy_data)
                    pred_act = self.forward(noisy_data, extra_act, node_mask)
                finally:
                    # Ensure hooks are removed even if forward raises
                    self.act_model_hooks.remove_hooks()

                pred_X_act = F.softmax(pred_act.X, dim=-1)
                pred_E_act = F.softmax(pred_act.E, dim=-1)

                G_1_pred_act = (pred_X_act, pred_E_act)

                R_t_X = self.rate_matrix_designer.compute_graph_rate_matrix(
                    t, node_mask, G_t, G_1_pred_act
                )[0]
                R_t_E = self.rate_matrix_designer.compute_graph_rate_matrix(
                    t, node_mask, G_t, G_1_pred_act
                )[1]

                # Interpolate in log-space between uncond and act-adjusted rates
                R_t_X = torch.exp(
                ...
                # Fallback: original guidance behavior (uncond vs pred)
                R_t_X = torch.exp(
                    torch.log(R_t_X_uncond + 1e-6) * (1 - guidance_weight)
                    + torch.log(R_t_X + 1e-6) * guidance_weight
                )
